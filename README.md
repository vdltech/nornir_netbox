...
For example, assume a device that is part of a site `Site 1` for which the slug `site-1`. NetBoxInventory2 will create a group with the name `site__site-1` and add the device to the group.

These groups can then be used in a groups file and you can add data to them, similar to how the SimpleInventory plugin works. More information on on the groups file can be found in [Nornir's documentation](https://nornir.readthedocs.io/en/latest/tutorial/inventory.html).
